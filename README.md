...
A sleek note-taking application that lets you quickly capture thoughts. Built with Next.js 16 (App Router), React 19, and Tailwind CSS v4. The app uses a SQLite database via Prisma ORM for persistent storage of users and notes.

## Features

- **Add Notes** — Type in the text field and press Enter or click "✨ Add Note" to create new entries
- **Edit Notes** — Click "✏️ Edit" on any note to change its content directly in an inline editor (WIP)
- **Delete Notes** — Remove unwanted notes with a single click on "🗑️ Delete" (WIP)
- **Timestamps** — Each note automatically displays its creation date
- **Change Log** — Edited notes show their last update date (WIP)
...
│   ├── login/route.ts  # Login authentication endpoint (POST /api/login)
│   └── notes/route.ts  # Notes endpoints (GET, POST)
...
- **`editText`** — The text content when editing a note (WIP)

When the user clicks "✨ Add Note" or presses Enter, the text is validated and a new note with a unique ID (based on `Date.now()`) is created.
